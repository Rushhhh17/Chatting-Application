...
public class Server  implements ActionListener {
 ...
 static JFrame f=new JFrame();//we could have directly extended Jframe in the server class but we need this to be static so the other method is to create its object.
    ...
    JPanel p1= new JPanel();//if you want to something above the frame we use panel and can devide also
      ...
      public void mouseClicked(MouseEvent ae){
        ...
        p1.add(moreOptions);
               ...
               JLabel name=new JLabel("Rishu Shukla");
...
f.setLocation(250,100); //by default the frame always appear on the left top side but to change that we use this.
...
panel.add(time);
    ...
    public static void main(String[] args) {
        new Server();
        try{
            ServerSocket skt=new ServerSocket(3001);//starts the server
            while(true){
               Socket s= skt.accept();//as we have to run the loop infinitely and accept all requests
                DataInputStream din=new DataInputStream(s.getInputStream()); //used to receive the input
              dout=new DataOutputStream(s.getOutputStream());//used to send the messages or output them
            while(true){
